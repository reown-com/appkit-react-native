...
export class AppKit {
  ...
  private async syncAuthConnector(provider: AppKitFrameProvider) {
    const activeNamespace = ChainController.state.activeChain as ChainNamespace;
    const isAuthConnected =
      (await StorageUtil.getConnectedConnectorId(activeNamespace)) ===
    ...
    if (isAuthConnected) {
      ...
      const namespace = await StorageUtil.getActiveNamespace();
      ...
      if (namespace) {
        if (isConnected && this.connectionControllerClient?.connectExternal) {
          ...
          // ChainController.setActiveConnector(connector); //todo check this
        } else if (
          (await StorageUtil.getConnectedConnectorId(namespace)) === ConstantsUtil.CONNECTOR_ID.AUTH
        ) {
          this.setLoading(false);
          this.setStatus('disconnected', namespace);
          StorageUtil.removeConnectedNamespace(namespace);
  ...
  private async setupAuthConnectorListeners(provider: AppKitFrameProvider) {
    ...
    provider.onNotConnected(async () => {
      ...
      if (isConnectedWithAuth) {
        ...
        StorageUtil.deleteConnectedConnectorId(namespace); //todo: check this
      }
      this.setLoading(false);
  ...
  private async syncConnectedWalletInfo(chainNamespace: ChainNamespace) {
    const connectorId = await StorageUtil.getConnectedConnectorId(chainNamespace);
  ...
  private async syncNamespaceConnection(namespace: ChainNamespace) {
    const connectorId = await StorageUtil.getConnectedConnectorId(namespace);
    const isEmailUsed = await this.authProvider?.getLoginEmailUsed();
  ...
  private async syncExistingConnection() {
    const connectedNamespaces = await StorageUtil.getConnectedNamespaces();
    try {
