import type { WalletConnector } from "../adapters/types";
...
export const ConnectorUtil = {
  async createConnector({ walletType, extraConnectors, projectId }: { walletType: string, extraConnectors: WalletConnector[], projectId: string }): Promise<WalletConnector> {
    ...
    if (walletType === "walletconnect") return WalletConnectConnector.create({ projectId, metadata: mockMetadata });
