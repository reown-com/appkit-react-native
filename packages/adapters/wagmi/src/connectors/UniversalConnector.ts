...
import { WcHelpersUtil } from '@reown/appkit-react-native';
import { StorageUtil } from '@reown/appkit-core-react-native';
import type { AppKit, AppKitOptions } from '@reown/appkit-react-native';
import {
  ConstantsUtil,
  type CaipNetwork,
  type ChainNamespace
  ...
  return createConnector<Provider, Properties, StorageItem>(config => ({
    ...
    async getProvider({ chainId } = {}) {
      ...
      if (chainId && currentChainId !== chainId && activeNamespace) {
        const storedCaipNetworkId = await StorageUtil.getStoredActiveCaipNetworkId();
